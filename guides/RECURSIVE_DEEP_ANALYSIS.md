...
> **做什么**: 对大型项目的每个模块递归执行完整三层分析，突破最大分析模式的深度瓶颈。
...
- 专项模板（按项目类型）→ LLM Agent 11 个 / Database 11 个 / Infrastructure 3 个
...
```bash
python3 $SKILL_DIR/scripts/verify-analysis.py <output-dir> --maximum
```
